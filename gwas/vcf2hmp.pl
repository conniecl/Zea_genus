...
close LI;
open OUT,">mex.genotype.hmp" or die "$!";
@list=();
foreach $chr(1..10)
{
            ...
            if($len==1)
            {
                ...
                foreach $i(@list)
                {
                    ...
                    if($brray[0] ne $brray[1])
                    {
                        if($brray[0] eq "1")
                        {
                            $allele=join("",$base{$brray[1]},$base{$brray[0]});
                        }
                        else
                        {
                            $allele=join("",$base{$brray[0]},$base{$brray[1]});
                        }
                    }
                    else
                    {
                    ...
                    #$allele=join("",$base{$brray[0]},$base{$brray[1]});
                    $geno{$allele}+=1;#print "$tmp[2]\t$i\t$tmp[$i]\t$allele\n";
                }
                foreach $key(keys %geno)
                {
                    $rate=$geno{$key}/($#list+1);
                    if($rate<=0.05||$rate==1)
                    {
                        $flag=1;
                    }
